...
/* ----------------------------------------------------------------
  ...
  if (first_call){
    ...
    /*[EMA] this "elif" was added by Ema*/
    #elif GEOMETRY == CYLINDRICAL && COMPONENTS == 1
     a12Bx2 = ARRAY_3D(NX3_MAX, NX2_MAX, NX1_MAX, double);
    #endif
    first_call = 0;
  }
...
/* --------------------------------------------------------------
   ...
   /*[EMA] this "elif" has been added by Ema*/
   #elif COMPONENTS == 1
    TOT_LOOP(k,j,i) a12Bx2[k][j][i] = 0.0;
   #endif
  #elif GEOMETRY == POLAR
   r   = grid[IDIR].x; rp  = grid[IDIR].xr;
   #if COMPONENTS >= 2
...
/*!
  ...
  /* ----------------------------------------------------
    ...
    BOX_LOOP(&box,k,j,i){
      ...
      /*[Ema] added by ema to debug*/
      //printf("a12Bx2[k][j][i+1]: %f, a12Bx2[k][j][i]: %f. i:%d, j:%d, k:%d", a12Bx2[k][j][i+1], a12Bx2[k][j][i], i,j,k);

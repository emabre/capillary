...
{
  int i,j,k;
  int vel_field, mag_field, curr_dens_field; /*[Ema] I add current density*/
  char header[512];
...
/* --------------------------------------------------------
  ...
  curr_dens_field = (strcmp(var_name,"jx1") == 0); /*[Ema] Line added by Ema*/
  if (vel_field || mag_field || curr_dens_field) { 
    DOM_LOOP(k,j,i){ 
    ...
    } else if (mag_field) { // [Ema] I added "if (mag_field)"
      sprintf (header,"\nVECTORS %dD_Magnetic_Field float\n", DIMENSIONS);
    } else if (curr_dens_field){ // [Ema] I added this third possiblity
      sprintf (header,"\nVECTORS %dD_Current_Dens_Field float\n", DIMENSIONS);
    }

  ...
  /* -- if any of the previous is true dump data to disk -- */
    ...
    if (check_dt || check_dn || check_dclock) {
      /*Added by [Ema]*/
        ...
        for (dd=0;dd<3;dd++) {
          if (d_correction[dd].Npoints != 0) {
            // TOT_LOOP(k,j,i) {
            //   for (nv=NVAR; nv--;) v[nv] = d->Vc[nv][k][j][i];
            // }
            *d_corrected = *d;
            ApplyMultipleGhosts(d_corrected, dd);
            #ifdef USE_ASYNC_IO
             if (!strcmp(output->mode,"single_file_async")){
               Async_BegWriteData (d_corrected, output, grid);
             }else{
               WriteData(d_corrected, output, grid);
             }
            #else
             WriteData(d_corrected, output, grid);
            #endif
